...
The recommended solution has been implemented. The client-side code has been modified to use the WebSocket flow exclusively for the "Take a Trip Down Memory Lane" button, with a fallback to the REST API for robustness. This eliminates the race condition and ensures users always see complete data in the summary.
